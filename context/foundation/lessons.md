...
- **DISCHARGED 2026-08-31 (same day)** — the root cause was two unscoped fixture inserts at bookings_rls.test.sql:36-43, which read `from public.specialist_profiles` with no `where` and therefore set up every specialist the database happened to contain. Adding `supabase/seed.sql` turned that latent assumption into a hard failure and made it findable. Both statements are now scoped by fixture id, and the suite passes twice in a row with no reset in between (141 assertions, exit 0, both runs). The rule above still stands as the verification habit — a reset is still the honest way to attribute a red suite — but the suite no longer depends on it.
