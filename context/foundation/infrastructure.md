...
| Missed cron run leaves stale "pending" bookings (FR-011) | Devil's advocate | L | M | Make auto-expire idempotent **and** enforce expiry lazily at read time (treat past-window pending as expired in queries); add a heartbeat/alert on the cron — **implemented in S-05**: `public.expire_stale_bookings()` is idempotent by construction, `public.bookings_view.effective_status` is the lazy half, and each transition function re-checks `expires_at` as a third layer. Moving the schedule off Cloudflare (see step 5 below) also removes the outage exposure this row was written about |
...
5. ~~**Wire the FR-011 cron**: add a `[triggers] crons` entry in `wrangler.jsonc` plus a `scheduled()` handler for the daily auto-expire; verify with `npx wrangler tail`.~~ **Superseded in S-05 — the expiry job runs in `pg_cron`, not on Cloudflare** (`supabase/migrations/20260807140000_booking_expiry_cron.sql`, every 15 minutes). Three reasons, in order of weight: (a) a Worker has no user session, so it would reach Postgres as `service_role` — the role that carries `BYPASSRLS` and whose legacy JWT for this project was exposed on 2026-08-07 — and that key would have to live in Worker secrets; (b) `@astrojs/cloudflare@14.1.5` has no `workerEntryPoint` option and `wrangler.jsonc` points `main` straight at the adapter's own entrypoint, which exports only `{ fetch }`, so adding `scheduled()` means hand-writing a build entry; (c) the 3-crons-per-Worker cap and the 2026-07-08 cron outage recorded in the risk register above. Running it in the database removes all three: the function executes as its owner, there is no key, and the schedule is versioned in a migration. Verify with `select * from cron.job_run_details order by start_time desc` rather than `wrangler tail`.
