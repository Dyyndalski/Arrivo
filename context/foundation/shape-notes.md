...
target_scale:
  users: enterprise        # user aims for 10k+ eventually
  qps: low                 # MVP launch traffic is small
  data_volume: small
timeline_budget:
  mvp_weeks: 3
  hard_deadline: 2026-08-04   # ~12 days from created date — tighter than mvp_weeks; see Open Question #9
...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: "primary side"
      decision: "demand-first — clients are the primary persona; specialists secondary"
    - topic: "core insight"
      decision: "home-visit-first, not salon-first (vs. Booksy-style incumbents)"
    - topic: "pain category"
      decision: "access/mobility barrier (clients) + premises-cost barrier (specialists)"
    - topic: "auth strategy"
      decision: "email + password; two distinct roles (client, specialist); no admin in MVP"
  frs_drafted: 15
...
> Seed idea: at-home beauty/hair/cosmetic services marketplace — connects
> specialists (hairdressers, beauticians) with clients who can't or don't want
> to travel to a salon. Web MVP. Source: idea-notes.md.
...
> Insight to sharpen later (Open Question): what specifically do salon-first
> incumbents (e.g. Booksy) get wrong about the home-visit flow — surfaced but
> not yet answered.
>
> Scale note (Phase 6 Socratic probe): user targets 10k+ users. The core rule
> (area-match + trust surfacing) doesn't change at scale, but two v1
> simplifications get riskier as volume grows — self-declared service areas
> (gaming) and thin trust signals. Verified areas + richer trust signals become
> more important the larger the marketplace gets.
...
Ran at end of Phase 7. Status: **accepted** — all gate elements present.

- Access Control: present.
- Business Logic (one-sentence rule): present.
- Project artifacts: present.
- Timeline-cost acknowledged: present (3-week target). Note: hard deadline
  (2026-08-04) is tighter than the estimate — see Open Question #9.
- Non-Goals: present.

No gaps to mirror into the PRD as blockers. The nine Open Questions are
downstream-resolution items, not shape gaps.

## Forward: tech-stack (informational — not a PRD concern)
- v2 candidates surfaced during scope-down and the Socratic round:
  maps/geocoding + routing for real travel-time; availability-calendar
  scheduling; free-text (moderated) reviews; free-text search; dual-role
  accounts. Captured so the downstream chain can pick them up; NOT part of
  the PRD.
...
7. **What is the auto-expire window for pending booking requests?** — Owner:
   user. Needs a concrete duration for FR-011 (e.g. 3 days).
...
9. **Deadline (2026-08-04) is tighter than the 3-week MVP estimate.** — Owner:
   user. ~12 days from the created date vs. a 3-week build. Either the deadline
   moves, the scope shrinks further, or the estimate is optimistic. Decide
   deliberately before build starts.
